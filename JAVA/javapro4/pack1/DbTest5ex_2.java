...
public class DbTest5ex_2 extends JFrame implements ActionListener{
	...
	private void layInit() {
		...
		lblResult.setSize(new Dimension(600, 50));
		panel3.add(lblResult);
		
		
		txtResult.setEditable(false);
		JScrollPane panel4 = new JScrollPane(txtResult);
...
//		add("South", panel3);
	...
	public void actionPerformed(ActionEvent e) {
		try {
			if(e.getSource() == btnFirst) rs1.first();
			else if(e.getSource() == btnPre) rs1.previous();
			else if(e.getSource() == btnNext) rs1.next();
			else if(e.getSource() == btnLast)	rs1.last();
